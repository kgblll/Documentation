...
%%%%%%%%%%%%%%%
% Title Page
\title{Gu\'ia docente:  uso de Dr.  Scratch para el desarrollo del pensamiento computacional}
...
Analizar un proyecto con Dr. Scratch es realmente sencillo. Para ello, hay que visitar el sitio web \url{http://drscratch.org} y elegir el proyecto que queremos analizar. La selecci\'on del proyecto puede realizarse tanto escribiendo la url del propio proyecto\footnote{La url del proyecto es parecida a http://scratch.mit.edu/projects/123456}, ideal si estamos trabajando con la versi\'on online de \texttt{Scratch}, o bien subiendo el archivo .sb o .sb2, en caso de que estemos trabajando con la versi\'on offline de la herramienta.

\begin{figure}[!h]
\centering
\includegraphics[width=0.7\textwidth]{img/portada.png}
\caption{P\'agina principal de Dr. Scratch}
\end{figure}
  
Tras elegir el proyecto y pulsar en el bot\'on de analisis, \texttt{Dr.}{\tiny{ }}\texttt{Scratch} muestra la ventana de resultados con informaci\'on sobre el grado de desarrollo de los diferentes aspectos del pensamiento computacional analizados, as\'i como informaci\'on sobre ciertos detalles que podr\'ian mejorarse en la implementaci\'on del proyecto.

\begin{figure}[!h]
\centering
\includegraphics[width=0.7\textwidth]{img/resultados.png}
\caption{P\'agina de resultados tras analizar un proyecto con Dr. Scratch}
\end{figure}
...
\caption{Fragmento de c\'odigo con nivel 0 en abstracci\'on.}
\end{figure}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.125\textwidth]{img/abs2.jpg}
\caption{Fragmento de c\'odigo con nivel 1 en abstracci\'on.}
\end{figure}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.125\textwidth]{img/abs3.jpg}
\caption{Fragmento de c\'odigo con bloques repetidos.}
\end{figure}
...
\caption{Funci\'on propia para evitar repetir c\'odigo.}
\end{figure}
...
\caption{Usando nuestra propia funci\'on.}
\end{figure}
...
Vamos a proponer un ejercicio para practicar la abstracci\'on. Construye un programa en \texttt{Scratch} en el que el personaje tenga que pintar las letras A, B y C.
\frameboxend
...
\caption{Fragmento de c\'odigo con un solo flujo.}
\end{figure}
...
\caption{Fragmentos de c\'odigo con dos flujos de ejecuci\'on.}
\end{figure}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.125\textwidth]{img/flow3.jpg}
\caption{Usando el bloque 'si'.}
\end{figure}
...
\frameboxbegin{Ejercicio}
Vamos a proponer un ejercicio para practicar el control de flujo. Construye un programa en \texttt{Scratch} en el que se gu\'ie paso a paso a un cocinero a la hora de realizar la receta de tu plato favorito.
\frameboxend
...
\caption{Usando el bloque 'si, si no'.}
\end{figure}
...
\frameboxbegin{Ejercicio}
Realiza un programa en Scratch que haga uso de condicionales y bloques "Y" y "O".
\frameboxend
\subsection{Paralelismo}
En la mayor\'ia de las creaciones \texttt{Scratch} se requiere un cierto nivel de paralelismo. Pero, ¿qu\'e es el paralelismo? El paralelismo es la posibilidad de que varias cosas ocurran al mismo tiempo. Por ejemplo, que dos personajes realicen una acci\'on al mismo tiempo, o que un personaje haga varias cosas a la vez.
\subsubsection*{¿C\'omo conseguir 1 punto?}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.125\textwidth]{img/paral1.jpg}
\caption{Bloque "al pulsar bandera verde".}
\end{figure}
...
\caption{C\'odigo que se ejecuta en paralelo.}
\end{figure}
...
\caption{Disparadores de eventos.}
\end{figure}
...
\caption{Usando bloque es 'esperar'.}
\end{figure}
\newpage
...
\caption{Usando env\'io de mensajes.}
\end{figure}
...
\caption{Otros bloques de sincronizaci\'on.}
\end{figure}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.25\textwidth]{img/data8.jpg}
\caption{Usando las listas.}
\end{figure}
...
\fullboxbegin
En resumen...
La representaci\'on de los datos nos ayuda a conocer algunas caracter\'isticas de nuestros personajes.
\fullboxend

\frameboxbegin{Ejercicio}
Realizar un programa en \texttt{Scratch} en el que se realice un sorteo.
\frameboxend

\subsection{Interactividad con el usuario}
Las instrucciones relacionadas con la interactividad con el usuario pueden ayudarte a que tus proyectos \texttt{Scratch} sean m\'as interactivos, es decir, que la persona que est\'e ejecut\'andolo pueda realizar acciones que provoquen nuevas situaciones en el proyecto. Por ejemplo, puedo utilizar el teclado o el rat\'on para mover un personaje, puede contestar preguntas o jugar con la webcam del ordenador, entre otras muchas posibilidades.
\subsubsection*{¿C\'omo conseguir 1 punto?}
El bloque m\'as b\'asico con el que puedes comenzar a trabajar la interactividad con el usuario es 'Al presionar la bandera verde':

\begin{figure}[!h]
\centering
\includegraphics[width=0.125\textwidth]{img/userinteractivity1.png}
\caption{Al presionar bandera verde.}
\end{figure}

¿C\'omo funciona este bloque? Cuando el usuario que quiere ejecutar tu proyecto \texttt{Scratch} pinche en la bandera verde, comenzar\'an a ejecutarse todos los bloques que se hayan colocado bajo el bloque ¿al presionar la bandera verde?. Puedes tener tantos programas como necesites que comiencen con este bloque.
\subsubsection*{¿C\'omo conseguir 2 puntos?}
Ahora que has comenzado a interactuar con el usuario que ejecute tu proyecto \texttt{Scratch}, quiz\'as puedes probar algunos de los siguientes bloques:
\begin{figure}[!h]
\centering
\includegraphics[width=0.25\textwidth]{img/userinteractivity2.png}
\caption{Bloques de interacci\'on con el usuario.}
\end{figure}
\newpage
Como ves, es posible permitir a los usuarios interactuar con tus personajes presionando teclas para que se muevan, escribiendo respuestas a preguntas y que tus personajes usen las respuestas del usuario, mover a los personajes con el rat\'on, o realizar acciones cuando el usuario pincha sobre un objeto.
\subsubsection*{¿C\'omo conseguir 3 puntos?}
Una de las posibilidades m\'as divertidas de \texttt{Scratch} en cuanto a la interactividad con el usuario es la posibilidad de que el usuario pueda utilizar su webcam o el micr\'ofono para interactuar con los personajes de \texttt{Scratch} o el sonido para provocar ciertas acciones.
\begin{figure}[!h]
\centering
\includegraphics[width=0.25\textwidth]{img/userinteractivity3.jpg}
\caption{Usando la c\'amara y el micr\'ofono.}
\end{figure}


Como ves en los ejemplos, podemos hacer que ciertos conjuntos de bloques se ejecuten cuando el sonido ambiente capturado por el micr\'ofono sea mayor que un determinado umbral, o cuando se produce movimiento del v\'ideo encima de un objeto o del fondo. Las posibilidades de estos bloques son muy grandes y permitir\'an que tus creaciones \texttt{Scratch} sean totalmente interactivas con los usuarios.

\fullboxbegin
En resumen...
Es importante trabajar la interactividad con el usuario para que \'este pueda comunicarse con la l\'ogica del proyecto. Ademas... ¡es m\'as divertido!
\fullboxend

\frameboxbegin{Ejercicio}
Historia interactiva. Realiza un proyecto en \texttt{Scratch} que narre una historia din\'amica, construida a partir de eventos.
Por ejemplo: comienza la historia y dependiendo del objeto en el que el usuario pulse, pasa una cosa u otra.
\frameboxend

\section{Promoviendo buenos h\'abitos de programaci\'on con Dr.  Scratch}
\subsection{Nombrado de objetos}
Al comenzar a programar con \texttt{Scratch} es bastante habitual dejar el nombre que \texttt{Scratch} asigna por defecto a los nuevos personajes (objeto1, objeto2...). Cuando tenemos pocos personajes, es bastante sencillo saber qu\'e personaje tiene cada nombre; no obstante, cuando el n\'umero de personajes crece, la cosa se complica. Por tanto, es muy bueno acostumbrarse a poner un nombre significativo (que reconozcamos f\'acilmente) a cada objeto de nuestros proyectos, ya que nuestros programas se leer\'an mejor y se programar\'an m\'as r\'apidamente. ¡Evita los nombres por defecto! 
\subsection{C\'odigo muerto}
El c\'odigo muerto son partes de los programas que no llegan nunca a ejecutarse. ¿C\'omo puede formarse c\'odigo muerto? Por ejemplo, cuando tienes un programa que no comienza con un bloque de tipo sombrero, o cuando hay un programa que est\'a esperando a que le llegue un mensaje que nadie env\'ia. 

\begin{figure}[!h]
\centering
\includegraphics[width=0.4\textwidth]{img/deadcode1.png}
\includegraphics[width=0.4\textwidth]{img/deadcode2.png}
\caption{C\'odigo muerto.}
\end{figure}


Estas situaciones suelen ser la causa de proyectos que no funcionan como se espera de ellos, as\'i que, ¡trata de evitar c\'odigo muerto en tus programas! 
\subsection{Inicializaci\'on de los atributos de los personajes}
Uno de los errores que m\'as repetimos todos los programadores al aprender a programar es no inicializar correctamente los atributos de los objetos. ¿Qu\'e son los atributos? Son caracter\'isticas de un personaje que pueden ser modificadas en la ejecuci\'on de un proyecto, como su posici\'on, su tama\~no, su color, su orientaci\'on...

Cuando tengas bloques que modifiquen un atributo de un personaje, siempre deber\'ias tener un programa que, al comenzar a ejecutar el proyecto, asigne el valor con el que queremos que comience. Por ejemplo, si se modifica la posici\'on de un personaje, deber\'iamos tener un bloque 'ir a' bajo 'al pulsar la bandera verde', para colocar al personaje en su posici\'on inicial. 

\begin{figure}[!h]
\centering
\includegraphics[width=0.4\textwidth]{img/init1.png}
\includegraphics[width=0.4\textwidth]{img/init2.png}
\caption{Inicializando atributos.}
\end{figure}

\subsection{Repetici\'on de c\'odigo}
Imagina que en un proyecto tenemos dos programas que se componen de los mismos bloques y en los que solo var\'ian los par\'ametros o valores de los bloques.

\begin{figure}[!h]
\centering
\includegraphics[width=0.25\textwidth]{img/dup1.png}
\includegraphics[width=0.25\textwidth]{img/dup2.png}
\caption{C\'odigo repetido.}
\end{figure}
\newpage
¿Qu\'e ocurrir\'ia si quisi\'eramos hacer una peque\~na modificaci\'on? Tendr\'iamos que modificar ambos programas, lo que complica el mantenimiento del c\'odigo. En estas situaciones es mucho m\'as adecuado que el programador cree un bloque propio que defina este comportamiento y que utilice este nuevo bloque en todos los programas donde lo necesite. 

\begin{figure}[!h]
\centering
\includegraphics[width=0.25\textwidth]{img/dup3.png}
\caption{Evitando la repetici\'on de c\'odigo.}
\end{figure}
De este modo, si se quiere realizar una modificaci\'on, tan solo hay que ir al bloque definido por el programador. 

\section{Conclusiones finales}
Este documento presenta una gu\'ia sobre el uso de Dr. Scratch como herramienta de apoyo para desarrollar el pensamiento computacional, que ha sido dise\~nada con el objetivo de ayudar a los docentes a comprender las diferentes dimensiones que componen esta competencia, y ofrecer ideas y estrategias para su trabajo en el aula con el alumnado.

Esta gu\'ia, como todo el proyecto Dr. Scratch, se comparte con una licencia libre y se encuentra disponible en un repositorio p\'ublico\footnote{https://github.com/kgblll/Documentation/tree/master/LibroDocentes}, de manera que animamos al lector a compartir sus opiniones y cr\'iticas para que podamos seguir mejorando el documento, y del mismo modo, le animamos a participar activamente en el desarrollo de la pr\'oxima versi\'on del mismo.

\appendix
\section{Soluci\'on a los ejercicios propuestos}
FIXME: Quiz\'as podr\'ian crearse estudios por cada uno de los ejercicios propuestos (que en principio solo tendr\'ian un proyecto programado por nosotros con la soluci\'on), de forma que animemos a la gente a compartir sus soluciones a\~nadiendo sus proyectos al estudio.
...
{\scriptsize
(cc) 2015 Gregorio Robles, Jes\'us Moreno Le\'on, Mari Luz Aguado, Eva Hu. \\
